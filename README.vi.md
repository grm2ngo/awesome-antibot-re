...
Danh sách hiện có case Akamai BMP/XP1M/Bot Manager 2.0, một snapshot Cloudflare JS challenge năm 2023, RE implementation của reCAPTCHA 2014, Imperva Reese84, AWS WAF, Trip.com, VM của một marketplace và Douyin. Case Cloudflare kết hợp hook runtime với Babel AST; InsideReCaptcha có disassembler/decompiler bytecode và phân tích XTEA nhưng là historical snapshot, không phải bypass hiện tại. Công cụ được nhóm theo AST/IR và instrumentation. Tài liệu tích hợp CAPTCHA, chuẩn TLS/HTTP, detection, accessibility và dataset nằm ở trang hỗ trợ.
...
Vòng cập nhật: tìm theo case/phương pháp RE → đọc nguồn → theo references/related/code → chống trùng → chấm điểm → tự phản biện → cập nhật README, ledger và queue. Mười tuyến RE có mục tiêu khám phá khoảng 50 hồ sơ mỗi tuyến trong [roadmap](docs/RE_CATALOG_ROADMAP.md), nhưng đây không phải quota nhận bài. `working` và PoW cần bằng chứng tái lập riêng. Ngưỡng vẫn ≥85 và phải qua mọi gate. Xem [báo cáo CAPTCHA/RE](reports/2026-09-12-captcha-research.md), [báo cáo Cloudflare snapshot](reports/2026-09-12-cloudflare-habr.md), [báo cáo sửa trọng tâm](reports/2026-09-12-re-focus.md) và [lịch duy trì](docs/AUTOMATION.md).
